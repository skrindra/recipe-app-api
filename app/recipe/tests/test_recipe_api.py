...
class PrivateRecipeAPITests(TestCase):
    ...
    def test_create_recipe_with_new_tags(self):
        ...
        payload = {
            ...
            'time_minutes': 12,
            'price': Decimal('7.88'),
            'tags': [{'name': 'South Indian'}, {'name': 'Biriyani'}],
        }
    ...
    def test_create_recipe_with_existing_tags(self):
        ...
        # create a payload with existing tag and new tag
        payload = {
            'title': 'Idli',
            'time_minutes': 4,
            'price': Decimal('5.55'),
    ...
    def test_create_tag_on_recipe_update(self):
        ...
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        # retrieve the tag that was created while recipe patch
    ...
    def test_update_recipe_assign_tag(self):
        ...
        # update the tag from Breakfast to Lunch
        payload = {'tags': [{'name': 'Lunch'}]}
        url = detail_url(recipe.id)
        res = self.client.patch(url, payload, format='json')
    ...
    def test_clear_recipe_tags(self):
        ...
        # clear the tags by updating with empty tags list
        payload = {'tags': []}
        url = detail_url(recipe.id)
        res = self.client.patch(url, payload, format='json')
    ...
    def test_create_recipe_with_existing_ingredients(self):
        """Test craeting a recipe with existing ingredients."""
    ...
    def test_create_ingredient_on_recipe_update(self):
        ...
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        new_ingredients = Ingredient.objects.get(user=self.user, name='Lemon')
        self.assertIn(new_ingredients, recipe.ingredients.all())

    def test_update_recipe_assign_ingredient(self):
        """Test assigning an ingredient on recipe update."""
        ingredient_old = Ingredient.objects.create(user=self.user, name='Tomato')
        recipe = create_recipe(user=self.user)
        recipe.ingredients.add(ingredient_old)

        ingredient_new = Ingredient.objects.create(user=self.user, name='Cherry')
        payload = {'ingredients': [{'name': 'Cherry'}]}
        url = detail_url(recipe.id)
        res = self.client.patch(url, payload, format='json')
    ...
    def test_clear_recipe_ingrdients(self):
        """Test clearing the ingrediets of the recipe."""
        ingredient = Ingredient.objects.create(user=self.user, name='Tomato')
        recipe = create_recipe(user=self.user)
        recipe.ingredients.add(ingredient)

        # patch with empty list of ingredients
        payload = {'ingredients': []}
        url = detail_url(recipe.id)
        res = self.client.patch(url, payload, format='json')
    ...
    def test_filter_by_tags(self):
        ...
        r1 = create_recipe(user=self.user, title='Gobi Manchurian')
        r2 = create_recipe(user=self.user, title='Brinjal samosa')
        tag1 = Tag.objects.create(user=self.user, name='vegitarian')
        tag2 = Tag.objects.create(user=self.user, name='vegan')
        r1.tags.add(tag1)
        r2.tags.add(tag2)
        r3 = create_recipe(user=self.user, title='Mardona Fish curry')
    ...
    def test_filter_by_ingredients(self):
        ...
        r1 = create_recipe(user=self.user, title='Gobi Manchurian')
        r2 = create_recipe(user=self.user, title='Brinjal samosa')
        ing1 = Ingredient.objects.create(user=self.user, name='Gobi')
        ing2 = Ingredient.objects.create(user=self.user, name='Brinjal')
